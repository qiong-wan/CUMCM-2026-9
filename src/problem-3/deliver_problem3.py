...
from solve_problem3 import (OUT, AUDIT_DIR, FIGURES_DIR, VALIDATION_DIR,
                            R, Environment, audit, dump_json)
...
def figures(data, mean_data, report, values):
    ...
    ax[1].plot(mean_data["times"] / 3600, mean_data["outputs"][:, 1, 0], color="#a45178", label="末段30分钟均值延拓")
...
def main():
    ...
    result_lines += ["",
        ...
        "",
        "表 5、CSV、工作簿数据和结束时间均来自 cases/production.npz 的同一状态序列。数值验收通过后才生成导出载荷；导出后由 verify_problem3.py --workbook 重读全部格值、格式、表头、时间、距离及表 5 对应点。最终工作簿验证状态见 review/workbook_verification.json，未生成该文件时不能声称 Excel 已通过检查。",
        "", "## 10. 验证状态及限制", "",
        "| 必要检查 | 状态 |", "|---|---|"]
