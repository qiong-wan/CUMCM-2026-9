...
数值上继承有限体积法、分块 Picard 迭代和 SDIRK2，但在每个隐式阶段同步更新半径、几何储存系数、环境边界及局部物性。全域事件在随体网格上检查，交付时再映射回固定实际距离；已落在药材外部的位置留空，并单独报告真实表面值。主方案沿用问题三的固定种子 AR(1) 环境构造方法，以末值、末段均值延拓作环境对照；另在统一末值环境下设置物性与几何的交叉对照，辨别两者对烘干时长的作用。
...
\begin{enumerate}
  ...
  \item 原始环境和半径记录之间采用分段线性插值。观测期后，问题三、四主方案均采用固定种子为 $0$ 的 AR(1) 环境，并以末 $30\,\si{min}$ 均值延拓作对照；问题四另设末值环境对照。半径不外推至附件 2 的 $72\,\si{h}$ 覆盖范围以外。
...
\end{equation}
式\eqref{eq:p4_radius_interp}严格通过全部观测点，且只在附件覆盖的 $0$ 至 $72\,\si{h}$ 内使用。附件 1 覆盖期内，$T_\infty(t)$ 与 $C_\infty(t)$ 仍按式\eqref{eq:linear_interp}插值；$4\,\si{h}$ 后主方案采用与问题三相同的 AR(1) 环境构造方法。取 $12600$ 至 $14400\,\si{s}$ 的 $31$ 条记录，对温度、水分浓度分别作线性去趋势，以残差的样本标准差估计波动幅度，以残差滞后一阶相关系数估计持续性。环境均值仍取该窗口的原始算术均值，而不外推拟合趋势。

记 $Y\in\{T,C\}$，将残差再标准化至零均值、单位总体方差后，有放回抽样得到新息 $\xi_{Y,m}$。在 $t_m=14400+60m$ 的网格上，按零初始状态生成未截断序列 $w_{Y,m}$，再得到用于边界输入的状态 $z_{Y,m}$：
\begin{equation}
\begin{aligned}
\phi_Y&=\min\{0.95,\max(0,\widehat{\mathrm{acf}}_{Y,1})\},\qquad w_{Y,-1}=0,\\
w_{Y,m}&=\phi_Yw_{Y,m-1}+\sqrt{1-\phi_Y^2}\,\xi_{Y,m},\\
z_{Y,m}&=\min\{4,\max(-4,w_{Y,m})\},\qquad
Y^{\mathrm{tail}}_m=\overline Y_{30}+\sigma_Yz_{Y,m}.
\end{aligned}
\label{eq:p4_ambient_tail}
\end{equation}
式\eqref{eq:p4_ambient_tail}中的截断作用于生成后的状态序列，不反馈到未截断递推；两通道分别抽样，未建立温湿交叉相关模型。当前取 $\sigma_T=0.155427\,\si{\celsius}$、$\sigma_C=1.29334\times10^{-4}\,\si{kg/kg}$，$\phi_T=0$、$\phi_C=0.0156216$；其中温度残差的负相关估计约为 $-0.092$，按所设非负持续性约束取零。均值沿用式\eqref{eq:p3_tail_mean}，计算使用未舍入统计量，种子固定为 $0$，幅度倍率为 $1$。

对 $t>14400\,\si{s}$，在相邻 $Y^{\mathrm{tail}}_m$ 之间作线性插值；观测终点本身仍取实测值，切换处允许小跳变。随机序列预先生成，局部事件重积分始终查询同一路径，避免反复抽样改变事件函数。图\ref{fig:p4_inputs_radius}展示观测环境、假设的后续环境及实测半径；这是一条可复现的边界情景，不代表观测延伸或多次随机试验的统计结论。
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{../../output/problem-4/image/01_inputs_and_radius.png}
  \caption{问题四的观测环境、随机延拓与实测半径历程}
...
\end{equation}
温度场另采用题设经验显热闭合，忽略潜热、迁移焓和压缩功，将式\eqref{eq:p4_thermal_properties}的有效热容作用于物质温度变化率，得到
\begin{equation}
...
\label{eq:p4_event}
\end{equation}
事件定位后取重新积分确认严格达标的上端点 $t_{\mathrm{end}}$ 作为交付结束时刻。逐区间线性重构的最大值位于端点，故检查全部节点可覆盖完整离散重构场；此处严格小于阈值的结论只针对该数值场。固定实际距离 $r_j$ 的输出按
\begin{equation}
...
每个阶段按其真实时刻查询 $R(t)$、环境边界和局部物性，并以分块 Picard 迭代交替求解温度、水分三对角系统；尺度化迭代变化与重新代入原非线性方程的缺陷均不超过 $10^{-11}$ 才接受阶段。每次线性化所得对称正定带状系统采用 Cholesky 分解求解。第一阶段以上一接受步速率外推作为初猜，仅在候选状态物理可行时启用；阶段最多迭代 $60$ 次，失败则拒绝整步并减半重算。

主计算采用 $N=25600$、时间倍率 $0.5$，共 $25601$ 个随体节点。步长在启动及 $4\,\si{h}$ 环境切换后由小到大递增，观测期内上限为 $2\,\si{s}$，后段为 $30\,\si{s}$；再截断至环境与半径插值节点、$60\,\si{s}$ 输出时刻和诊断时刻。实际步长介于 $5\times10^{-5}$ 与 $30\,\si{s}$，共接受 $14719$ 步且无拒绝步，阶段最多迭代 $8$ 次。首次发现式\eqref{eq:p4_event}变号后，从同一未达标状态局部重积分并二分，直到夹逼宽度不超过 $10^{-3}\,\si{s}$；另以 $10^{-5}\,\si{s}$ 容差及局部子步加密作事件核验。

\subsection{求解结果}

在式\eqref{eq:p4_ambient_tail}所设随机环境及实测收缩条件下，主方案的临界烘干时长为 $51.06553993\,\si{h}$，按四位小数报告为 $\boldsymbol{51.0655\,\mathrm{h}}$。严格达标结束时刻为
\begin{equation}
t_{\mathrm{end}}=183835.944214\,\si{s}=51.06554006\,\si{h}.
\label{eq:p4_end_time}
\end{equation}
该时刻的全域判据为
\begin{equation}
C_{\max}=0.149999999981678\,\si{kg/kg},\qquad
0.15-C_{\max}=1.8322\times10^{-11}\,\si{kg/kg}.
\label{eq:p4_strict_margin}
\end{equation}
最大值位于圆柱中心。式\eqref{eq:p4_strict_margin}仅确认未舍入离散场达标，其中的微小裕度不包含空间、时间离散误差。表\ref{tab:p4_moisture}给出每 $6\,\si{h}$ 的代表性结果；固定距离位于当时药材外部时不列数值，表面列对应实际变化的半径。
\begin{table}[H]
\centering
...
51.06554006 & 0.1500 & 0.1413 & 0.1081 & 0.0525 \\
\bottomrule[1.2pt]
\end{tabular}}
\end{table}
...
图\ref{fig:p4_space_time}将含水率展开至实际半径与时间平面，灰色区域位于药材外部，白色虚线为 $0.15\,\si{kg/kg}$ 等值线。等值线由外向内推进，表明外层达标时中心仍可高于阈值；该线由保存场插值得到，只用于展示，终止时间仍由式\eqref{eq:p4_event}在未舍入全场上定位。结合式\eqref{eq:p4_fixed_domain_pdes}可知，水分演化同时受局部扩散系数和时变半径控制。
\begin{figure}[!htbp]
  \centering
...
水分检验将参考权重平均含水率的变化与按 SDIRK2 阶段权重独立累计的表面通量比较；显热检验则分别累计变热容储存率和表面热输入，不将热容乘温度的首末差当作储热变化。根据本轮计算报告，水分、显热平衡的最大相对缺陷及两场最大尺度化迭代变化、原方程缺陷为
\begin{equation}
\begin{aligned}
e_{C,\mathrm{bal}}&=3.8314\times10^{-15},&
e_{T,\mathrm{bal}}&=3.1041\times10^{-14},\\
e_{\mathrm{iter}}&=9.9965\times10^{-12},&
e_{\mathrm{eq}}&=1.6151\times10^{-13}.
\end{aligned}
\label{eq:p4_balance_metrics}
\end{equation}
在 $60\,\si{s}$ 及以后的诊断时刻，以二阶单侧差分独立重构表面梯度，热、湿 Robin 最大绝对残差分别为 $7.5524\times10^{-6}\,\si{W/m^2}$ 和 $1.5305\times10^{-12}\,\si{(kg/kg)\,m/s}$。启动角点由均匀初值与非零失水驱动力的不相容产生，单列诊断，不能据此声称初始瞬间已满足表面梯度条件。

加密比较采用同一随机环境路径，在共同保存时刻将粗网格分段线性场映射至全部细网格点，再取两场差的最大绝对值。$0$ 至 $60\,\si{s}$ 的启动段单列，表\ref{tab:p4_refinement}所列全场差仅统计 $t>60\,\si{s}$；时长差取细方案减粗方案。
\begin{table}[!htbp]
\centering
\caption{问题四随机环境主方案的相邻离散差}
\label{tab:p4_refinement}
{\small
\begin{tabularx}{\textwidth}{Lccc}
\toprule[1.2pt]
加密设置 & 最大含水率差/(\si{kg/kg}) & 最大温度差/\si{\celsius} & 时长差/\si{s} \\
\midrule
$N:12800\to25600$，倍率 $0.5$ & $1.6338\times10^{-6}$ & $5.1845\times10^{-8}$ & $-0.0513$ \\
倍率 $0.5\to0.25$，$N=25600$ & $1.1913\times10^{-6}$ & $5.2301\times10^{-4}$ & $+0.0055$ \\
\bottomrule[1.2pt]
\end{tabularx}}
\end{table}

两组含水率差均低于 $5\times10^{-5}\,\si{kg/kg}$，时长差绝对值均低于 $5\,\si{s}$。时间加密的温度差满足随机环境采用的 $10^{-3}\,\si{\celsius}$ 验收阈值，但高于确定性情景的 $5\times10^{-5}\,\si{\celsius}$，因此不能将四位小数显示解释为温度场已具有相应精度。当前空间与时间各只有一组相邻比较，可说明已检验层级之间差异较小，尚不足以估计观察收敛阶或给出连续解的严格误差界。

事件夹逼宽度为 $9.1553\times10^{-4}\,\si{s}$，收紧根容差后临界时刻变化 $4.2558\times10^{-4}\,\si{s}$；局部改用两子步、四子步后，变化分别为 $-3.5763\times10^{-5}$、$-7.1526\times10^{-6}\,\si{s}$。独立 BDF 对照只覆盖 $N=80$、$0$ 至 $60\,\si{s}$，温度与含水率末态差分别为 $3.75\times10^{-9}\,\si{\celsius}$、$3.89\times10^{-8}\,\si{kg/kg}$；其空间算子与主方法相同，不能视作全程独立验证。这些局部差异也不构成 $\pm1\,\si{s}$ 的累计误差保证。

为考察观测截止后的环境假设，保持附录 4 物性、实测收缩、$N=25600$ 和时间倍率 $0.5$ 一致，分别采用随机波动、末值和末段均值延拓。图\ref{fig:p4_ambient_comparison}展示三条最大含水率曲线及临界时刻附近的差别。主方案为 $51.06553993\,\si{h}$；保持末值 $50.165\,\si{\celsius}$、$0.04986\,\si{kg/kg}$ 时为 $50.82420540\,\si{h}$，较主方案缩短 $14.4801\,\si{min}$；末段均值情景为 $51.06910108\,\si{h}$，较主方案延长 $12.8201\,\si{s}$。随机与末值情景同时改变了环境中心水平和波动，二者差异不能全部归因于随机波动；随机与均值的对照则只反映当前单条路径的影响。图中的均值上下两倍标准差仅标示波动尺度，不是干燥时长或环境预测的置信区间。
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{../../output/problem-4/image/08_ambient_fluctuation.png}
  \caption{问题四的后续环境情景与全域达标时间对照}
  \label{fig:p4_ambient_comparison}
\end{figure}

为进一步辨别几何与物性的作用，将四组交叉对照统一置于末值环境，结果见表\ref{tab:p4_cross_scenarios}。其中附录 4 实测收缩的 $50.8242\,\si{h}$ 是确定性对照，不能用随机主方案的 $51.0655\,\si{h}$ 替代后再作单因素归因。
\begin{table}[!htbp]
\centering
\caption{统一末值环境下的物性与几何交叉对照}
\label{tab:p4_cross_scenarios}
{\small
\begin{tabularx}{\textwidth}{ccrL}
\toprule[1.2pt]
物性 & 几何 & 临界时长/\si{h} & 计算终点状态 \\
\midrule
附录 3 & 固定半径 & 57.1694 & 全域达标 \\
附录 3 & 实测收缩 & 25.1362 & 全域达标 \\
附录 4 & 固定半径 & -- & $72\,\si{h}$ 内未达标，$C_{\max}=0.2237\,\si{kg/kg}$ \\
附录 4 & 实测收缩 & 50.8242 & 全域达标 \\
\bottomrule[1.2pt]
\end{tabularx}}
\end{table}

由表\ref{tab:p4_cross_scenarios}可见，给定物性时，实测收缩通过缩短传递距离促进达标；给定收缩历程时，附录 4 相较附录 3 显著延缓达标。由式\eqref{eq:p4_diffusivity}、\eqref{eq:p4_fixed_domain_pdes}可知，含水率下降引起的扩散减弱与半径减小引起的尺度变化共同作用，不能按末半径比例缩放时长或将两种效应简单相加。附录 4 固定半径在数据覆盖期内未达标，也不意味着其永远不能达标。

数值平衡、边界重构、相邻加密和短时 BDF 对照为给定情景下的离散一致性提供证据；均匀内部收缩、观测期后环境及经验物性仍构成模型适用范围的限制。因此，$51.0655\,\si{h}$ 是给定实测半径与固定随机路径下的条件预测，既不预测新工况下的收缩本构，也不直接构成实际工艺的安全终点。
