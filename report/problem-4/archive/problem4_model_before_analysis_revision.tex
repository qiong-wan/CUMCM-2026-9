...
预热阶段的温度与含水率均受时变烘房环境驱动，且热扩散快于水分扩散。首先用传输特征数判断内部梯度能否忽略，再由圆柱微元守恒建立径向传热传质模型，采用有限体积法计算表层响应和中心滞后。

\subsection{问题二分析}

附录 3 使热物性随含水率变化，扩散系数同时依赖温度与含水率，因而两场形成双向耦合。本问保留问题一的几何、初边值结构与通量离散，将局部物性嵌入隐式迭代，并从题设初值重算前 $3\,\si{h}$，检验升温是否先于充分脱水完成。

\subsection{问题三分析}

问题三沿用问题二的控制方程，将给定时域计算改为全域含水率首次达标事件。新增工作是构造 $4\,\si{h}$ 以后的环境情景、稳定推进长期扩散过程，并对全场最大含水率进行事件夹逼；时长的不确定性需区分数值离散与边界延拓两类来源。

\subsection{问题四分析}

问题四同时引入实测半径变化和附录 4 物性。由均匀骨架收缩假设建立物质速度，再从干物质与水分守恒推导随体方程，将移动域映射为固定区间。求解后重新映射实际输出位置，并以物性与几何的交叉对照解释时长变化。

\subsection{总体建模思路}

四问依次形成“径向传递基础模型、局部物性耦合、全域达标事件、收缩域传递”的递进关系。四问共享圆柱通量离散与隐式迭代，物性更新、终点判断及几何变化按各问条件引入。各问均从题设均匀初值独立计算，问题一、二或三的末场不作为下一问的初值。
